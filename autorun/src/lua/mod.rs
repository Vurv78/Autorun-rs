...
#[cfg(executor)]
pub fn run_async(realm: Realm, code: String) -> Result<(), RunError> {
...
pub fn run<S: AsRef<str>>(l: LuaState, script: S) -> Result<(), LuaEnvError> {
...
pub fn set_path<P: AsRef<Path>>(l: LuaState, p: P) {
	let path = p.as_ref();

	println!("Setting path {} {:#?}", lua_gettop(l), dump_stack(l));
...
pub use env::push_fenv;

pub fn prepare_base_env<F: Fn(LuaState)>(l: LuaState, func: F) {
	println!("Preparing base env");

	// stack = {}
	lua_createtable(l, 0, 0); // stack[1] = {}
	lua_createtable(l, 0, 0); // stack[2] = {}

	println!("b4 {} {:#?}", lua_gettop(l), dump_stack(l));
...
pub fn prepare_with_env(
	l: LuaState,
	env: &AutorunEnv,
	prep: Option< fn(LuaState) >,
) {
	prepare_base_env(l, |l| {
		// Autorun located at -2
		println!("Prepare_with_env {} {:#?}", lua_gettop(l), dump_stack(l));
