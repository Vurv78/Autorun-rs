...
pub fn initMenuState(state: LuaState) -> Result<(), detour::Error> {
	use crate::detours::{joinserver, JOINSERVER_H};
...
static ILLEGAL: Lazy<Regex> = Lazy::new(|| {
	Regex::new(r#"[\x{003F}\x{003C}\x{003E}\x{005C}\x{003A}\x{002A}\x{007C}\x{0022}]"#).unwrap()
});
...
static WINDOWS_RESERVED: Lazy<Regex> =
	Lazy::new(|| Regex::new(r"^(?i)(con|prn|aux|nul|com[0-9]|lpt[0-9])(\..*)?$").unwrap());
